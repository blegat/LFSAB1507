...
We note that the regularisation algorithm is much more sensitive to both type of noise added in the original image. The other two methods are also affected but the deblurring is not compromised (provided that the initial noise is not too important).
...
\caption{Picture deblurred with Lucy}
\label{fig:Lulu}
\end{subfigure}
~
\begin{subfigure}{0.32\textwidth}
...
We see that in this case the compression significantly reduces the computation time with little visible differences between the deblurred image with and without compression (more artefacts for reguralisation with the processing process). Deblurring with the Wiener is inconclusive here, this observation will be discussed in section of the color influence (????????).
...
\caption{Deblurring (Lucy) for artificial blur of $19$ pixels with $k=3$ (length estimation = $19$ pixels)}
\label{fig:LB19E19K3}
\end{subfigure}
~
\begin{subfigure}{0.32\textwidth}
...
\caption{Deblurring (Lucy) for artificial blur of $20$ pixels with $k=3$ (length estimation = $36$ pixels)}
\label{fig:LB20E36K3}
\end{subfigure}
~
\begin{subfigure}{0.32\textwidth}
...
\caption{Deblurring (Lucy) for artificial blur of $21$ pixels with $k=3$ (length estimation = $21$ pixels)}
\label{fig:LB21E21K3}
\end{subfigure}
...
\label{fig:LB20E20K2}
\end{subfigure}
\caption{Deblurring (Lucy) for artificial blur of $20$ pixels with $k=2$ (length estimation = $20$ pixels)}
\end{figure}
...
\caption{Pics for artificial blur of $19$ pixels}
\label{fig:PicsLucy19}
\end{subfigure}
~
\begin{subfigure}{0.32\textwidth}
\includegraphics[{width= \textwidth}]{../Images/Results/plaque/20/PicsLucy20.png}

\caption{Pics for artificial blur of $20$ pixels)}
\label{fig:PicsLucy20}
\end{subfigure}
~
\begin{subfigure}{0.32\textwidth}
\includegraphics[{width= \textwidth}]{../Images/Results/plaque/20/PicsLucy21.png}

\caption{Pics for artificial blur of $21$ pixels}
\label{fig:PicsLucy21}
\end{subfigure}
\caption{PicsLucy A COMPLETER}
\end{figure}
...
Below are the results obtained with the algorithm Lucy-Ridchrdson (with compression) for the photo of Lena and the gray-scaled one corresponding, both artificially blurred with an angle of 50 degrees and a length of 80 pixels:
