...
\label{eq:FeHF}
\end{equation} 
 where $N$ is the DF transform of a noise $n$ with mean $0$. We want to estimate $\hat{F}$ based on the observed image $G$ and a filter $L$ which still need to be defined, i.e. 
 \begin{equation}
...
However, this filter needs an estimation of the noise and image variances, which are difficult to estimate. For further details on this estimation, please refer to section \ref{sec:NSREstimation}.
...
The term $L(k,l)$ is chosen as a high pass filter in order to minimize the high-frequencies, i.e. the roughness in the solution. Indeed noise has big compnents in the high frequencies as it is a big change between two pixels. 
